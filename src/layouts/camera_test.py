...
def window(x=0, y=0, w=1920, h=1080, c=0):
    ...
    camera_test = [[sg.Text('Cam Number'), sg.InputCombo(avail_cameras, size=(4,1), key="-CAM_NUM-"), sg.Button('Update'), sg.Button('Blurry? Test Resize'), sg.Text('This will crash sometimes im working on figuring out why lol')], #sg.In(default_text=c, key="-CAM_NUM-",size=(2, 1), )
    ...
    while True:
        ...
        if event == 'Update':
            ...
            cap = Capture(c)
            print("Trying Device %s at %s x %s (This is the native size reported by the camera...)" % (c, cap.get_width(), cap.get_height()))
            ret, frame = cap.read()
            if ret:
